...
return this.each(function(){
  var i=0, c=$(this), s=c.children(), o=$.extend({speed:500,timeout:4000},c.data()), f=o.fx!='scroll',
    ...
    tfn=function(){setTimeout(tx,t);}, scss = $.extend({position:a,top:0}, f?{right:0}:{right:w}, o.scss);
  if (c.css(p)=='static')
    css[p]='relative';
  ...
  function tx(){
    ...
    if (f){
      ...
      a.animate({right:-w},z,function(){
        a.hide();
      });
      b.css({'right':w,display:'block'}).animate({right:0},z,tfn);
    }
    i = i==(l-1) ? 0 : (i+1);
  }
});
